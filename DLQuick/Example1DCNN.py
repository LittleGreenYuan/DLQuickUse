...
#from Netmobels import BPNet#从Netmobels中导入BPNet这个类
#继承于torch的自定义数据集
...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1  = nn.Conv1d(in_channels=1,out_channels=10,kernel_size=3,stride=2,padding=0, dilation = 1,groups=1,bias=True)
        self.max_pool1 = nn.MaxPool1d(kernel_size=3,stride=2)
        #这里由于没有认真计算输出格式，所以去掉了这一层，理论上需要计算输入输出的格式
        #self.conv2 = nn.Conv1d(in_channels=10,out_channels=10,kernel_size=3,stride=2,padding=0, dilation = 1,groups=1,bias=True)
        #self.max_pool2 = nn.MaxPool1d(kernel_size=3,stride=2)
        self.liner1 = nn.Linear(50,32)
...
def readData():
    ...
    train=np.reshape(train,[np.shape(train)[0],1,np.shape(train)[1]])
...
def transformONNX(Outpth):
    ...
    dummy_input = torch.randn(1,1,24,device=device).double()
    input_names = ["input"]
    output_names = ["output"]
